...
type VirtualClusterLister interface {
	...
	List(selector labels.Selector) (ret []*v1.VirtualCluster, err error)
...
// List lists all VirtualClusters in the indexer.
func (s *virtualClusterLister) List(selector labels.Selector) (ret []*v1.VirtualCluster, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1.VirtualCluster))
	})
	return ret, err
}
...
type VirtualClusterNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*v1.VirtualCluster, error)
...
// List lists all VirtualClusters in the indexer for a given namespace.
func (s virtualClusterNamespaceLister) List(selector labels.Selector) (ret []*v1.VirtualCluster, err error) {
...
// Get retrieves the VirtualCluster from the indexer for a given namespace and name.
func (s virtualClusterNamespaceLister) Get(name string) (*v1.VirtualCluster, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1.Resource("virtualcluster"), name)
	}
	return obj.(*v1.VirtualCluster), nil
}
